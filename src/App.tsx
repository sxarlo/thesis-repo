...
function normalizeStatus(st: string | null | undefined): QueueStatus {
...
type AssignNextResult = { queue: QueueEntry[]; called: QueueEntry; counter: string } | { reason: 'empty' | 'full' }

function assignNextPending(queue: QueueEntry[]): AssignNextResult {
...
function App() {
  ...
  const handleDocRequest = useCallback((e: React.FormEvent) => {
    ...
    const entry = addToQueueFn(reqName, '', 'document-request', reqDocType)
  ...
  }, [reqName, reqDocType, addToQueueFn, showNotif])

  const handleClaim = useCallback((e: React.FormEvent) => {
    e.preventDefault()
    if (!claimName) { showNotif('Please fill in your name.', 'info'); return }
    const entry = addToQueueFn(claimName, '', 'claim-document', claimDocType || null)
    setLastQueueEntry(entry)
    setClaimName(''); setClaimDocType('')
    showNotif(`Queue #${entry.number} assigned!`, 'success')
  }, [claimName, claimDocType, addToQueueFn, showNotif])
  ...
  const handleStatusCheck = useCallback(() => {
    if (!statusInput.trim()) { showNotif('Enter a queue number.', 'info'); return }
  ...
  useEffect(() => {
    autoCallRef.current = autoCallFn
  })
  ...
  const filteredQueue = db.queue.filter(q => {
    if (q.status.trim().toLowerCase() !== queueFilter) return false
    if (queueSearch) {
                ...
                <div className="dashboard-grid">
                  <div className="dashboard-card">
                    ...
                    <div className="dashboard-card-body">
                      {[...db.queue].filter(q => q.status !== 'cancelled').reverse().slice(0, 8).map(q => {
                ...
                <div className="dashboard-card">
                  ...
                  <div className="dashboard-card-body" style={{ overflowX: 'auto' }}>
                    <table className="queue-table">
                      ...
                      <tbody>
                        {db.documents.filter(d => d.status !== 'cancelled').length === 0 ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: 30, color: 'var(--gray-400)' }}>No requests.</td></tr> :
                          db.documents.filter(d => d.status !== 'cancelled').map(d => <tr key={d.id}>
                    ...
                    <div style={{ maxWidth: 500 }}>
                      <div className="form-group">
                        ...
                        <select style={{ padding: '10px 14px', border: '2px solid var(--gray-200)', borderRadius: 8, fontSize: 14, fontFamily: 'var(--font)', width: '100%' }}
                          ...
                          onChange={e => setSettingsForm(f => ({ ...f, autoCallNext: e.target.value === 'true' }))}>
                      ...
                      <div className="form-group">
                        ...
                        <input type="number" min={1} max={60}
                          ...
                          onChange={e => setSettingsForm(f => ({ ...f, interval: e.target.value === '' ? 0 : parseInt(e.target.value, 10) || 0 }))}
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        ...
                        <button className="btn btn-danger" onClick={resetSystem}>Reset Data</button>
                      </div>
                    </div>
                  </div>
                </div>
